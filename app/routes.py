...
from app._shared.services import (
    ...
    hash_password,
    generate_and_send_password_changed,
)
...
def contact_us(json_data):
    ...
    html = mailer.generate_email_text(
        "support_response.html", context={"name": data["name"]}
    )
...
def reset_password(json_data):
    ...
    if not user_type:
        return bad_request(
            f" 'user_type' is a required args. it should be either 'staff' or 'student' "
        )
...
def change_password(json_data):
    ...
    try:
        confirmed_user = jwt.decode(
            data["confirmation_code"], app.config["SECRET_KEY"], algorithms=["HS256"]
        )
