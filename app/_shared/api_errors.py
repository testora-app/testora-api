...
def unapproved_account(
    message="Your account has not been approved. Contact your school administrator",
):
    return response_builder(419, message)


def premium_only_feature(
    message="This feature is only available to premium users. Kindly upgrade to premium to gain access",
):
...
class BaseError(Exception):
    ...
    def to_dict(self):
        ...
        """
        return response_builder(
            status_code=self.error_code, message=self.message, data=self.payload
        )
...
class DatabaseError(BaseError):
    ...
    def __init__(
        self,
        message="Something went wrong! Our team has been informed and are working on it.",
    ):
...
class PermissionDeniedError(BaseError):
    ...
    def __init__(
        self, message="You do not have permission to do that!", error_code=403
    ):
