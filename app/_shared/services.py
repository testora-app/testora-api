...
def generate_and_send_reset_password_email(
    ...
    reset_code = (
        FRONTEND_BASE_URL
        + "/change-password/?confirmationCode="
        + jwt.encode(payload=payload_data, key=app.config["SECRET_KEY"])
    )

    html_body = mailer.generate_email_text(
        "password_reset.html", {"reset_code": reset_code, "name": name}
    )
    text_body = mailer.generate_email_text(
        "password_reset.html", {"reset_code": reset_code, "name": name}
    )
...
def generate_and_send_password_changed(email, name):
    ...
    mailer.send_email(
        [email], "Your  Password Has Been  Successfully Changed", html, html=html
    )
...
def generate_access_token(
    ...
    payload_data = {
        ...
        "exp": datetime.datetime.now(datetime.timezone.utc)
        + datetime.timedelta(days=0, seconds=7200),
        "iat": datetime.datetime.now(datetime.timezone.utc),
    }

    token = jwt.encode(payload=payload_data, key=app.config["SECRET_KEY"])
    return token
...
def set_current_user(
    user_type="admin", user_id=-1, user_email=None, school_id=-1, **kwargs
):
