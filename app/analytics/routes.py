...
def topic_performance(query_data):
    ...
    student_id, subject_id = query_data.get('student_id', None), query_data.get('subject_id', None)
    results = ssr_manager.get_topic_performance(student_id=student_id, subject_id=subject_id)
    ...
    best_performing = sorted(performance, key=lambda x: x['high_count'], reverse=True)[:2]
    worst_performing = sorted(performance, key=lambda x: x['low_count'], reverse=True)[:2]
    return success_response(data={'best_performing_topics': best_performing, 'worst_performing_topics': worst_performing})