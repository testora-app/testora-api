...
class TestReadNotificationsRoute:
    ...
    def test_put_read_notifications_multiple_ids(
        ...
        """Test PUT /notifications/read/ with valid notification IDs."""
        payload = {
            "data": {
                "notification_ids": [sample_notification.id]
            }
...
class TestTestNotificationsRoute:
    ...
    def test_get_test_notifications_with_admin_auth(
        self, client, auth_headers, sample_recipient, mock_pusher
    ):
        """Test GET /test-notifications/ with admin auth sends test notifications."""
        response = client.get('/test-notifications/', headers=auth_headers)

        assert response.status_code == 200

    def test_get_test_notifications_without_auth(self, client, mock_pusher):
        """Test GET /test-notifications/ without auth returns 401."""
        response = client.get('/test-notifications/')

        assert response.status_code == 401

    def test_get_test_notifications_with_student_auth(self, client, student_headers, mock_pusher):
        """Test GET /test-notifications/ with student auth returns 403."""
        response = client.get('/test-notifications/', headers=student_headers)
